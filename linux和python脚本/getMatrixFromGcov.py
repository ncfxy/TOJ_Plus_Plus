...
def getTestResult(sourceDir,x):
	resultFileName = sourceDir + "\\outputs.csv"
	resultFile = open(resultFileName, 'r')
	resultContent = resultFile.readlines()
	return resultContent[x-1][0]


def getMatrixFromGcov(version,programPath,programName,testNum):
	sourceDir = programPath
	sourceName = sourceDir+"\\"+programName
	...
	# 获取所有可执行行号
	executableLineNum = 0
	fileName = sourceName + str(1) + ".cpp.gcov"
	...
	for x in range(0,int(testNum)):
		...
		fileName = sourceName + str(x) + ".cpp.gcov"
		print(fileName)
...
if len(sys.argv) < 4:
    ...
    sys.exit(0)
programPath = sys.argv[1] #programName
programName = sys.argv[2]
lenn = sys.argv[3] # number of versions
testNum = sys.argv[4]# number of test case
for index in range(1,int(lenn)+1):
    getMatrixFromGcov(index,programPath,programName,testNum)
    print(index)
